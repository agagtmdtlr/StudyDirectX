...
Mesh::Mesh(void* vertices, UINT vertexStride, UINT vertexCount, void* indices, UINT indexCount)
{
	// Create an vertex buffer
	{
		...
		const HRESULT hr = RenderContext::GetRenderContext()->device->CreateBuffer(&bufferDesc,&vertexBufferData,vertextBuffer.GetAddressOf());
		if (FAILED(hr))
		{
	...
	// Create an index buffer
	{
		...
		const HRESULT hr = RenderContext::GetRenderContext()->device->CreateBuffer(&bufferDesc, &indexBufferData, indexBuffer.GetAddressOf());
		if (FAILED(hr))
		{
